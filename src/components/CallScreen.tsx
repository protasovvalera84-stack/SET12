...
export function CallScreen({ open, type, contactName, contactAvatar, onEnd }: CallScreenProps) {
  ...
  // Auto-connect after 2 seconds (simulating ringing)
  useEffect(() => {
    if (!open) return;
  ...
  // Request camera/mic when video call starts
  useEffect(() => {
    if (!open || !isVideoOn) {
      if (localStream) {
    ...
    navigator.mediaDevices
      ...
      .catch(() => {
        // Camera not available -- fall back to audio
        setIsVideoOn(false);
      });
      ...
      <div className={`relative flex flex-col flex-1 ${isFullscreen ? "" : "md:max-w-lg md:max-h-[700px] md:rounded-3xl md:border md:border-border/40 md:shadow-elegant"} overflow-hidden glass-strong`}>
        ...
        <div className="flex items-center justify-between px-5 py-4 z-10">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              className="hidden md:flex rounded-lg p-1.5 hover:bg-surface-hover transition-colors"
            >
              ...
              <div className="w-full h-full max-h-[60vh] rounded-3xl bg-black/30 border border-border/30 flex items-center justify-center overflow-hidden">
                <div className="flex flex-col items-center gap-4">
                  ...
                  <p className="text-sm text-muted-foreground">
                    {callState === "ringing" ? "Connecting camera..." : "Remote camera"}
                  </p>
                </div>
              </div>

              {/* Local video (picture-in-picture) */}
              <div className="absolute bottom-4 right-4 w-28 h-40 md:w-36 md:h-48 rounded-2xl overflow-hidden border-2 border-primary/40 shadow-glow">
                <video
                  ref={localVideoRef}
                  autoPlay
                  muted
        ...
        <div className="z-10 px-6 py-8">
          <div className="flex items-center justify-center gap-4">
            ...
            {/* Video toggle */}
            <CallButton
              icon={isVideoOn ? <VideoOff className="h-5 w-5" /> : <Video className="h-5 w-5" />}
              label={isVideoOn ? "Camera Off" : "Camera On"}
              active={!isVideoOn && type === "video"}
              onClick={toggleVideo}
            />
            ...
            <button
              onClick={handleEnd}
              className="flex h-16 w-16 items-center justify-center rounded-full bg-destructive text-destructive-foreground shadow-lg hover:scale-105 transition-all"
            >
            ...
            {/* Speaker */}
            <CallButton
              icon={isSpeaker ? <VolumeX className="h-5 w-5" /> : <Volume2 className="h-5 w-5" />}
    ...
    <button onClick={onClick} className="flex flex-col items-center gap-1.5">
      <div className={`flex h-12 w-12 items-center justify-center rounded-full transition-all hover:scale-105 ${
        active
          ...
          : "bg-secondary/80 text-muted-foreground border border-border/40 hover:bg-surface-hover"
      }`}>
        {icon}
      </div>
